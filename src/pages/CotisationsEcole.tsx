...
import { getAppelCotisationsEcole, createAppelCotisationEcole, updateAppelCotisationEcole, getContacts } from "../lib/api";
import type { AppelCotisationsEcole, Contact } from "../lib/api";
import { DataTable } from "../components/ui/data-table";
import { Button } from "../components/ui/button";
import { Plus, Pencil } from "lucide-react";
import { useToast } from "../hooks/use-toast";
import { AppelCotisationEcoleForm } from "../components/AppelCotisationEcoleForm";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function CotisationsEcole() {
  const [cotisations, setCotisations] = useState<AppelCotisationsEcole[]>([]);
  const [contacts, setContacts] = useState<Contact[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedCotisation, setSelectedCotisation] = useState<AppelCotisationsEcole | null>(null);
  ...
  const handleSubmit = async (data: any) => {
    try {
      if (selectedCotisation) {
        await updateAppelCotisationEcole(selectedCotisation.idliste, data);
        toast({
          title: "Succès",
          description: "Cotisation mise à jour avec succès",
        });
      } else {
        await createAppelCotisationEcole(data);
        toast({
          title: "Succès",
  ...
  const columns: ColumnDef<AppelCotisationsEcole>[] = [
    {
  ...
  return (
    <div className="container mx-auto py-10">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Cotisations École</h1>
